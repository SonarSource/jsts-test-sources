...
function getReturnKind(node, checker) {
    ...
    var contextual = isFunctionExpressionLike(node) ? tryGetReturnType(checker.getContextualType(node)) : undefined;
    var returnType = contextual !== undefined ? contextual : tryGetReturnType(checker.getTypeAtLocation(node));
    ...
    return ReturnKind.Value;
    function tryGetReturnType(fnType) {
        if (fnType === undefined) {
            return undefined;
        }
